{
  ...
  options.t4ko.zsh.extendedConfig.enable = lib.mkEnableOption "the workstation Zsh configuration";

  config = let
    extendedConfig = config.t4ko.zsh.extendedConfig.enable;
  in {
    xdg.configFile = lib.mkIf extendedConfig {
    ...
    programs.zsh = {
      ...
      envExtra =
        ...
        ''
        + lib.optionalString extendedConfig ''
          export ZRCDIR="$ZDOTDIR/rc"
          export LOCAL_ZSH_DIR="$HOME/.config/local/zsh"
        '';

      initContent = lib.mkIf extendedConfig (lib.mkMerge [
        (lib.mkOrder 560 ''
