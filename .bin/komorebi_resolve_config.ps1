...
#
# canonical な komorebi.json は dotfiles 側 (setup_windows.ps1 で
# $HOME\komorebi.json にリンク) を読み込み、リポジトリは書き換えない。
# 検出に失敗した場合は canonical のパスをそのまま返す（フォールバック）。

$ErrorActionPreference = "Continue"

# Canonical 設定ファイル（setup_windows.ps1 が $HOME\komorebi.json をシンボリックリンク化）
$canonicalPath = Join-Path $env:USERPROFILE "komorebi.json"
if (-not (Test-Path -LiteralPath $canonicalPath)) {
    # リンクが無い場合はリポジトリ直接を見る
    $canonicalPath = "$env:USERPROFILE\Project\github.com\t4ko0522\dotfiles\.config\komorebi\komorebi.json"
}
