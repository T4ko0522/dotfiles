---
name: at-doc-reviewer-codex
description: at-doc-writer が書いたドキュメントを Codex CLI 経由で独立レビューする。Phase 3R で使用。at-doc-reviewer-opus と二票化する。
model: sonnet
...
2. オーケストレーターから受け取った SLUG を変数に代入し、変更要旨 / 関連 diff / 実装メモを標準入力で渡す。`<slug>` をリテラル展開しない。ドキュメント差分は `git diff --name-only` から **計画ドキュメント (`docs/plans/`) を除外** して抽出する:
   ...
   {
     cat <<'PROMPT'
   以下は at-doc-writer の変更要旨 / 実ドキュメント差分 / 関連実装メモです。
